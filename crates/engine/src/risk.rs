...
impl RiskManager {
    ...
    /// Settle an open position: realize P&L into bankroll, update peak/drawdown,
    /// daily loss, and the kill-switch. No-op if the ticker isn't open.
    pub fn on_settlement(&mut self, ticker: &str, won: bool) {
        let Some(idx) = self.state.open.iter().position(|p| p.ticker == ticker) else {
            return;
        };
