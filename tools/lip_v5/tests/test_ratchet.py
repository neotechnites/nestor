...
class TestFloorQ(LipTestCase):
    ...
    def test_a_venue_whose_whole_pool_is_below_the_floor_is_None(self):
        """No q clears it — funding it is the PayPal error in miniature."""
        self.assertIsNone(RT.floor_q_contracts(rho=0.4, S=50, window_h=8.0))
        self.assertIsNone(RT.floor_q_usd(0.4, 50, 0.30, 8.0))
...
class TestOutOfReach(LipTestCase):
    ...
    def test_a_reading_below_the_entry_floor_is_neither_verify_nor_disagree(self):
        ...
        verdict, ratio, d = RT.apply_reading(v, reading_usd=0.0, projection_usd=1.50)
