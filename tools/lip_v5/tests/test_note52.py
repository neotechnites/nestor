...
class TestBookReinstatement(LipTestCase):
    ...
    def test_a_racing_first_pass_ADJUDICATES_NOTHING_and_keeps_every_rung(self):
        """(a) THE 0-OF-40 INCIDENT.  init runs ~10 s after boot, racing a programs feed
        still paginating and a bucket discovery has already drained.  Neither is a judgement
        about a rung, so nothing may be dropped."""
        snap = [{"ticker": self.TKR, "side": "bid", "price": 0.06, "q": 40},
                {"ticker": self.TKR + "B", "side": "bid", "price": 0.06, "q": 10}]
        r, ex, tk, NOW_ = self._rig(snapshot=snap)
        r.classifier.close_ts[self.TKR + "B"] = NOW_ + 16 * 3600
        ex.programs = lambda cursor=None: (200, {"incentive_programs": [],
                                                 "next_cursor": "more"})
        r.m.bucket.tokens = 0.0
        r.m.bucket.b = 0.0                     # discovery drained the lane
        r.init(NOW_, nestor_state={"open_order_tickers": [], "position_tickers": []})
        self.assertEqual(ex.placed, [])
        self.assertEqual(len(r.pending_reinstate), 2,
                         "rungs were convicted by a clock, not by a gate")

    def test_the_next_pass_with_a_FULL_map_places_them(self):
        """The other half: the queue drains from the loop once the answers exist."""
        snap = [{"ticker": self.TKR, "side": "bid", "price": 0.06, "q": 40}]
        r, ex, tk, NOW_ = self._rig(snapshot=snap)
        ex.programs = lambda cursor=None: (200, {"incentive_programs": [],
                                                 "next_cursor": "more"})
        ...
        r.init(NOW_, nestor_state={"open_order_tickers": [], "position_tickers": []})
        self.assertEqual(ex.placed, [])
        self.assertEqual(len(r.pending_reinstate), 1)
        # …the feed finishes, the bucket refills, the loop turns
        ex.programs = lambda cursor=None: (200, ex._programs)
        r.scanner.last_scan = None             # the cadence lets the next scan through
        r.iteration(NOW_ + 1)
        self.assertTrue(ex.placed, "the pending rung never reached the wire")
        self.assertEqual(r.pending_reinstate, [])

    def test_a_rung_a_REAL_gate_refused_never_retries(self):
        """(b) A verdict is a verdict: a far-settling rung leaves the queue on the first
        pass and no later pass reconsiders it."""
        snap = [{"ticker": self.TKR, "side": "bid", "price": 0.06, "q": 40}]
        r, ex, tk, NOW_ = self._rig(snapshot=snap, close_h=C.SETTLE_HORIZON_H + 100)
        r.init(NOW_, nestor_state={"open_order_tickers": [], "position_tickers": []})
        self.assertEqual(r.pending_reinstate, [], "a decided rung must leave the queue")
        r.iteration(NOW_ + 1)
        self.assertEqual(ex.placed, [])
        self.assertEqual(r.reinstate_why.get("close_horizon"), 1)

    def test_the_snapshots_own_deadline_drops_whatever_is_left(self):
        """(c) The queue cannot outlive the freshness the snapshot was trusted for: past
        REINSTATE_MAX_AGE_S the board has moved and the rest is re-derived honestly."""
        snap = [{"ticker": self.TKR, "side": "bid", "price": 0.06, "q": 40}]
        r, ex, tk, NOW_ = self._rig(snapshot=snap, snapshot_age=60.0)
        ex.programs = lambda cursor=None: (200, {"incentive_programs": [],
                                                 "next_cursor": "more"})
        r.init(NOW_, nestor_state={"open_order_tickers": [], "position_tickers": []})
        self.assertEqual(len(r.pending_reinstate), 1)
        r.reinstate_pass(NOW_ + C.REINSTATE_MAX_AGE_S + 1)
        self.assertEqual(r.pending_reinstate, [])
        self.assertTrue([l for l in self.logs_of("reinstate_stale") if l.get("dropped")])

    def test_the_running_tally_names_the_gate_each_pass(self):
        snap = [{"ticker": self.TKR, "side": "bid", "price": 0.06, "q": 40}]
        r, ex, tk, NOW_ = self._rig(snapshot=snap)
        ex.programs = lambda cursor=None: (200, {"incentive_programs": [],
                                                 "next_cursor": "more"})
        r.init(NOW_, nestor_state={"open_order_tickers": [], "position_tickers": []})
        row = self.logs_of("reinstated")[-1]
        self.assertEqual(row["pending"], 1)
        self.assertEqual(row["refused"].get("program_map_incomplete"), 1)
