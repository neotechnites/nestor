...
# VENDORED INTO lip_v5, 2026-07-28, from tools/lip_maker_v4/ws_feed.py @ 6018a37.
# spec §3.5: "inherit v4's ws_feed.py on its merits ... kept VERBATIM: the 3-agreement gate
# before a WS book may price anything, 60 s re-proof, `unit_mismatch` naming of a
# dollars/cents slip, per-market fallback to REST on stale/gapped/corrupt, breadth 6 -> 32
# only while connected."
#
# THE ONLY EDIT is this import head.  The coupling surface to the host module was verified by
# grep before vendoring and is exactly FOUR symbols -- `M._now`, `M.price_str`, `M.log` and
# `Auth` -- all of which lip_v5.runtime provides with identical semantics.  v5 must not import
# lip_maker_v4 at runtime: v4 is FROZEN and deployed separately, so a live coupling would mean
# a v4 edit silently changing v5, and a v5 deploy needing v4's tree on the box.
from . import runtime as M
from .runtime import Auth                        # noqa: F401  (re-exported; see ws_auth_headers)
