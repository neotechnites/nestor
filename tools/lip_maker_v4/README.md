...
0. **`python3 -m unittest discover tools/lip_maker_v4/` — 317 tests, must print `OK`.** Run it on the
...
600s and PAGES on disagreement (`position_divergence_detected`) — it does **not** freeze or cancel:
`RECON_FREEZE_ENABLED = False`, because settlement-index lag (R171: 41 min) would trip it ~4x per
settled market. Treat a page as "look at this market by hand"; nothing is blocked. No auto-import
(the endpoint has no cost basis). See the constant for the three guards required before arming.
Verify the parser against the live account any time with
`python3 lip_maker_v4.py --recon-once` (read-only): `rows` must equal `parsed`.
